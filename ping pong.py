...
class PingPong:
    ...
    def _check_pos_ball(self):
        if self.posy >= 44:self.y1 = 1; self._check_lifes()
        elif self.posy == 0:self.y1 = 0
        if self.posx == 60:self.x1 = 1
        elif self.posx == 0:self.x1 = 0
        
    def _check_direction_ball(self):
        if self.y1:self.posy -= 1
        else:self.posy += 1
        if self.x1: self.posx -= 1
        else: self.posx += 1
    ...
    def _log_system(self, cbal, crocket, score):
        if cbal: print(f"ball posx:{self.posx} ball posy:{self.posy}")
        if crocket: print(f"rocket posx:{self.rocketx} rocket posy:{self.rockety}")
        if score: print(self.score)
